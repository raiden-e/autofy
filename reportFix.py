...
def main():
    ...
    description = f'"My playlist got reported like {report_count} times now, which is really wicked 🔥" -raplaylist_iden_e'

    _spotify.playlist_change_details(playlist_id, name="Hard DnB 🔥 Drum and Bass", description=description)

    with open(pic_path, "rb") as pic:
        # Make sure the image size does not exceed 256 kb or so
        _spotify.playlist_upload_cover_image(f"spotify:playlist:{playlist_id}", base64.b64encode(pic.read()))
